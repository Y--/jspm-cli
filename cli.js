...
(function() {
  ...
  function showInstructions() {
    ...
    ui.log('\n'
      ...
      + '\n'
      + 'jspm bundle moduleA + module/b [outfile] [--minify] [--no-mangle] [--inject] [--skip-source-maps] [--source-map-contents] [--cjs-to-es5] [--amd-to-es5]\n'
  ...
  switch(args[0]) {
    ...
    case 'bundle':
      options = readOptions(args, ['inject', 'yes', 'skip-source-maps', 'minify',
          'no-mangle', 'hires-source-maps', 'no-runtime', 'inline-source-maps', 'source-map-contents', 'cjs-to-es5', 'amd-to-es5' ], ['format', 'global-name', 'globals']);
    ...
    case 'compile':
      options = readOptions(args, ['transpile', 'minify', 'removeJSExtensions', 'yes', 'cjs-to-es5', 'amd-to-es5'], ['map', 'format']);
      if (options.yes)
        ui.useDefaults();
      if (options.map) {
